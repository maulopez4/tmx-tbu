...
<div id="add_form_title"><i class="icon-plus-sign icon-large"></i> Set Rework/Disposition</div>     
<hr>
                    <!--------------------------------------------------------------------------------------------------------------->
                        <!--- Workstation --->
                        <?php
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
    ...
    <tbody>	
			...
			<td><input type="hidden" name="workorder_workstation" id="workstation" value="<?php echo $h;?>" /><?php echo $h;?> - <?php echo $paint["paintcode_description"];?></td>
</tr>
	</tbody>
</table>
            ...
            <tr>
                ...
                <td width="33%" style="text-align: center; vertical-align: top;">
                  Select Disposition/Rework:
                    ...
                    <!--- Defect Origin --->
                    <?php
                        $query="SELECT * FROM reworks WHERE rework_workstation_code = '$a'";
                        $sql=$db->prepare($query);
                        $sql->execute();
                        $reworks = $sql->fetchAll();
                    ?>
                <!--------------------------------------------------------------------------------------------------------------->
                  <select name="workorder_rework" id="rework" Required>         
                    <option value="" disabled selected>Set  Disposition/Rework</option>    
                        <?php foreach($reworks as $rework): ?>
                            <option value="<?php echo $rework["rework_code"]; ?>"><?php echo $rework["rework_code"]; ?> - <?php echo $rework["rework_description"]; ?> </option>
                    <?php endforeach; ?>
                  </select>
                ...
                <td width="33%" style="text-align: center; vertical-align: top;">
                    <!--- Submit Button --->
                    <button type="submit" formMethod="post" formAction="report_entry_save.php" class="btn btn-warning btn-block btn-large" style="float:center; width:150x; height:50px;"><i class="icon icon-bell icon-large"></i> Report</button>                        
                </td>
            </tr>
</table>    
</form>